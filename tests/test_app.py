...
class TestEncryption:
    ...
    def test_messages_are_stored_encrypted(self, client):
        ...
        client.post("/messages",
            json={"content": "secret text", "recipient": "alice"},
            headers=auth(token),
        )
...
class TestMessaging:
    ...
    def test_user_sees_only_their_messages(self, client):
        ...
        client.post("/messages", json={"content": "alice→bob",   "recipient": "bob"},   headers=auth(alice_token))
        client.post("/messages", json={"content": "charlie→bob", "recipient": "bob"},   headers=auth(charlie_token))
        client.post("/messages", json={"content": "bob→alice",   "recipient": "alice"}, headers=auth(bob_token))

        alice_contents   = {m["content"] for m in client.get("/messages", headers=auth(alice_token)).json()}
        bob_contents     = {m["content"] for m in client.get("/messages", headers=auth(bob_token)).json()}
        charlie_contents = {m["content"] for m in client.get("/messages", headers=auth(charlie_token)).json()}

        assert alice_contents   == {"alice→bob", "bob→alice"}
        assert bob_contents     == {"alice→bob", "charlie→bob", "bob→alice"}
        assert charlie_contents == {"charlie→bob"}
